...
When multiple plan files exist, the resolver tracks them and provides tab navigation. Tabs are available when 2+ categories are tracked. Categories: `"design" | "implementation" | "plan" | "ai-parsed"`.

**ai-parsed tab workflow:**
1. Plan chat starts → `beginSession()` clears trackedFiles
2. Claude writes PLAN.md → resolver tracks as `"plan"` category → single tab (no tab strip)
3. User clicks Form Plan → `ai-parsed-plan.md` generated by claudeloop
4. Resolver adds `"ai-parsed"` as a new category — **does not clear other categories**
5. Tab strip appears with both `"Plan"` and `"AI Parsed"` tabs; ai-parsed auto-switches to active
6. User can switch between original plan and AI-parsed version

**Deletion:** When `ai-parsed-plan.md` is deleted, it is pruned from trackedFiles on the next resolve cycle (no longer in candidates). Other categories remain.

**Transient stat failure:** If `ai-parsed-plan.md` is in candidates but `statFile()` fails (file mid-write), it is tracked using the candidate's `mtimeMs` as a fallback birthtimeMs so the tab appears immediately.
...
**Sources:** `src/core/planChatMarker.ts`, `src/planInterceptWatcher.ts`, `src/planInterceptInstaller.ts`, `resources/oxveil-plan-intercept.sh`

The plan intercept system intercepts `ExitPlanMode` tool calls and presents the user with options via `AskUserQuestion` in the terminal: execute with Oxveil, run critic agents first, or continue planning.
...
When the user selects "Form Plan with Oxveil" via `AskUserQuestion`, Claude writes a trigger file that the extension watcher picks up.
...
- **2026-06-25**: Added `src/mcp/logTail.ts` — ring buffer wrapping `console.log/warn/error` (500 lines / 64 KB cap). Side-effect imported at top of `extension.ts` so activation-time logs are captured. Exposes `getLogTail()` for new MCP `/log-tail` endpoint. No state machine behavior changed.
- **2026-06-28**: `formPlan` adds `fs.mkdir({ recursive: true })` before `writeFile(.claudeloop/PLAN.md)` — prevents ENOENT in fresh worktrees where `.claudeloop/` doesn't exist yet. No state machine behavior changed.
- **2026-06-28**: Added `oxveil.confirmPlan` command — dismisses the AI plan-verification dialog and proceeds with plan formation (same as clicking "Continue As-is"). No new states; bypasses the dialog modal only.
