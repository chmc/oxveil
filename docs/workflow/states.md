...
| `state-changed` → `done` | Stop elapsed timer, derive view from sidebar; **if `view === "completed"` (all phases done): delete `.claude/plans/` session plan file, clear `ai-parsed-plan.md`, reset `planDetected`/`cachedPlanPhases`/`planUserChoice`** | StatusBar (`done` or `stopped` via `deriveViewState`), LiveRunPanel (`onRunFinished("done"` or `"stopped")`), Sidebar (via `buildSidebarState()`), context key `oxveil.walkthrough.hasRun=true`, archive refresh; **on full completion: plan file deleted, sidebar returns to `empty` view** |
