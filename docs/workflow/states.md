...
| `phases-changed` | Update panels, notify on completions/failures (failures deduplicated per phase — only first failure per run notified); phase list HTML rendered with `session.status` (not hardcoded `"running"`) so stopped/failed state is reflected correctly | DependencyGraph, ExecutionTimeline, LiveRunPanel, StatusBar (current phase update), Sidebar (progress update) |
