...
session_path=$(echo "$marker_content" | grep -o 'session=.*' | head -1 | cut -d= -f2-)

# Find SESSION.md
session_md=""
if [ -n "$session_path" ] && [ -f "$session_path/SESSION.md" ]; then
  session_md="$session_path/SESSION.md"
else
  # Fall back to newest session
  session_md=$(find "${CLAUDE_PROJECT_DIR:-.}/verification-sessions" -name "SESSION.md" -type f 2>/dev/null \
    | xargs ls -t 2>/dev/null | head -1) || session_md=""
fi
