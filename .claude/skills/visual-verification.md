...
0. **Pre-flight** — Run pre-flight checks from recipes. Platform, permissions, `code` CLI, stale EDH cleanup. Create session folder: `verification-sessions/YYYYMMDD-HHMMSS-{title}/screenshots/`. Initialize SESSION.md.
...
2. **Interact** — Use osascript to send keystrokes to EDH window (Cmd+Shift+P → command). Log each action to SESSION.md. Wait for UI to settle.
